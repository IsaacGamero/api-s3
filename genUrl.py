...
def lambda_handler(event, context):
    ...
    if not bucket or not key:
        return {'statusCode': 400, 'body': json.dumps({'error': 'Falta bucket_name u object_key'})}

    if op not in ('upload', 'download'):
        return {'statusCode': 400, 'body': json.dumps({'error': 'operation debe ser upload o download'})}
